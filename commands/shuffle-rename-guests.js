  ...
  async execute(interaction) {
    try {
      ...
      if (notifyOption && Object.keys(failedUsers).length === 0) {
        const successMessage = `Users renaming successful! :trophy: \nIf you do not see the changes, please refresh the page or app.`;
        await interaction.channel.send(successMessage);
      }
    } catch (error) {
      const errorMsg = `An error occurred while executing the "shuffle-rename-guests" command: ${error}`;
      // console.error(errorMsg);
      await interaction.editReply({
        content: `An error occurred while executing the "shuffle-rename-guests" command. :sob:`,
        ephemeral: true,
      });
    }
  },
